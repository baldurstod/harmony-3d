...
export class TimelineObserver extends EventTarget {
	static #instance: TimelineObserver;
	constructor() {
	...
	*/
	...
	propertyAdded(element: TimelineElement, propertyName: string, type: TimelinePropertyType, value: any) {
		this.dispatchEvent(new CustomEvent(PROPERTY_ADDED, { detail: { element: element, name: propertyName, type: type, value: value } }));
	}

	propertyChanged(element: TimelineElement, propertyName: string, oldValue: any, newValue: any) {
